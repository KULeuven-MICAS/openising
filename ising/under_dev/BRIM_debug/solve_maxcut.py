...
from ising.under_dev.BRIM_debug.BRIM_afoakwa import BRIMafoakwa
from ising.under_dev.BRIM_debug.BRIM_current import BRIMcurrent
from ising.under_dev.BRIM_debug.BRIM_roychowdhury import BRIMroychowdhury
from ising.under_dev.BRIM_debug.Multiplicative import Multiplicative
...
def test_afoakwa():
    J = np.array([[0, -1, -1 / 2, 1], [0, 0, 0, -3], [0, 0, 0, 0], [0, 0, 0, 0]])
    h = np.zeros((4,))
...
def test_afoakwa2():
    ...
    for N in N_list:
        ...
        for run in range(nb_runs):
            ...
            ballisticSB().solve(problem, initial_state, num_iterations, c0, dt, file=logfile_SB, bit_width=32)
            logger.info(f"run {run} of bSB done")

            # Solve with Afoakwa
            logfile_BRIM = logtop / f"BRIM_afoakwa_N{N}_run{run}.log"
            BRIMafoakwa(
                problem, initial_state, num_iterations, dt, C, logfile_BRIM, stop_criterion, Temp, cooling_rate, seed
            )
    ...
    best = np.array(best)
    plot_energy_dist_multiple_solvers(
        logfiles, xlabel="problem_size", fig_name="small_energy_all_test.png", save_folder=figtop, best_found=best
    )
...
def test_small():
    ...
    for N in N_list:
        ...
        _, energy = ballisticSB().solve(problem, initial_state, num_iterations, 1.0, dt, 1.0, logfile)
        ...
        logfiles.append(logfile)
        current_logfiles.append(logfile)
        _, energy = BRIMafoakwa(
            problem, initial_state, num_iterations, dt, C, logfile, max_change, Temp, cooling_rate, seed
        )
    ...
    plot_energy_dist_multiple_solvers(
        logfiles, xlabel="problem_size", fig_name="small_energy_all.png", save_folder=figtop, best_found=best
    )
...
def test1():
    ...
    for N in N_list:
        ...
        logfile = logtop / f"bSB_N{N}.log"
        logfiles.append(logfile)
        ballisticSB().solve(problem, initial_state, num_iterations, c0=1.0, dtSB=0.25, file=logfile, a0=1.0)
        plot_state_continuous(logfile, figname=f"bSB_N{N}.png")
...
def test2():
    ...
    for num_iterations in iter_list:
        ...
        logfiles.append(logfile)
        current_logfiles.append(logfile)
        ballisticSB().solve(problem, initial_state, num_iterations, c0=c0, dtSB=0.25, file=logfile, a0=1.0)
        ...
        plot_energies_multiple(
            current_logfiles,
            figName=f"{bench}_energy_iter{num_iterations}.png",
            save_folder=figtop,
    ...
    plot_energy_dist_multiple_solvers(
        logfiles,
        xlabel="num_iterations",
        fig_name=f"{bench}_energy_all.png",
        save_folder=figtop,
