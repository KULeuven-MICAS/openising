...
def initialisation():
    ...
    for i in range(nb_runs):
        LOGGER.info(f"Random initialisation run {i}")
        y = np.random.uniform(-0.1, 0.1, (model.num_variables,))
        _, energy = ballisticSB().solve(model, y, num_iter, c0, dt, a0)
        optim_energies_rand.append(energy)
    
    for i in range(nb_runs):
        LOGGER.info(f"Deterministic initialisation run {i}")
        y = 0.1*np.ones((model.num_variables,))
        _, energy = ballisticSB().solve(model,y, num_iter, c0, dt, a0)

    plt.figure()
    plt.hist(optim_energies_rand, bins=10, alpha=0.5, edgecolor='black', label='Random Initialisation')
    plt.hist(optim_energies_det, bins=10, alpha=0.5, edgecolor='black', label='Deterministic Initialisation')
    plt.axvline(best_found, ymin=0, ymax=100, linestyle='--', label=f"best found: {best_found}")
    plt.xlabel('Energy')
    plt.ylabel('Frequency')
    plt.savefig(figtop / "SB_initialisation.png")
