...
benchmarks = {"MCP": "G11", "MIMO":"MIMO", "TSP": "burma14", "QKP":"jeu_100_25_1"}
best_energies = { "TSP": 3323, "QKP":-18558, "MIMO": 0., "MCP": -564}
...
for app, benchmark in benchmarks.items():
    ...
    file_SB = orig  / f"{app}/{app}_bSB_energies.csv"
    ...
    energies_bSB = np.loadtxt(file_SB)
    ...
    dfSB = pd.DataFrame({"application": app, "technique":"bSB", "energy": energies_bSB})
    ...
    df = pd.concat([df, dfSB, dforig, dfrand]) #  , dfgrad, dffreq
...
plt.savefig(save_path / "boxplot_energies_SB_orig_flipping_techniques.png", bbox_inches="tight")
plt.close()
