...
class ATSPParserStage(Stage):
    ...
    def run(self) -> Any:
        ...
        LOGGER.debug(f"Parsing ATSP benchmark: {self.benchmark_filename}")
        graph: nx.Graph
        best_found: float | None
        graph, best_found = self.ATSP_parser(benchmark=self.benchmark_filename)
    ...
    def ATSP_parser(benchmark:pathlib.Path|str) -> tuple[nx.DiGraph, float]:
        ...
        graph.name = name
        best_found = ATSPParserStage.get_optim_value(benchmark)
