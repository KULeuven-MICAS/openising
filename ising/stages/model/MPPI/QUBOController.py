import numpy as np

class QUBOController:
    """QUBO Controller instance"""
    def __init__(self, config: dict):
        """! Initialise the controller.

        @type config: dict
        @param config: qubo config dictionary
        """
        self.config = config

        action_dim = len(config.R) if config.R else len(config.R_diag)
        # This builds the binary encoding from config
        self.EE, self.mapping = self.continuous_to_binary_mapping(
            action_dim,
            config.HORIZON_LENGTH,
            config.K,
            config.N_BITS,
            config.action_scales,
            config.scale_multiplier
        )

        # Slack variable from config
        self.Q_slack = config.Q_slack

        # Build the trajectory expanded action and state penalties (supports dynamics / not used)
        self._RR = self.build_R_trajectory(None, None)
        self._QQ = self.build_Q_trajectory(None, None)

    @property
    def RR(self):
        return self._RR

    @property
    def QQ(self):
        return self.Q_slack * self._QQ

    def build_Q_trajectory(self, x_bar, u_bar):
        """! Build Q along trajectory.
        Notes
        -----
        Can be used to make dynamic range optimization based on rollout (SLAM)
        Kept empty in current iterations.

        @type x_bar: np.ndarray
        @param x_bar: nominal state trajectory
        @type u_bar: np.ndarray
        @param u_bar: Action sequence that generates x_bar

        @rtype: np.ndarray
        @return: Upscaled and weighted state cost matrix (L * state_dim, L*state_dim)
        """
        # Extract single state cost matrix from config
        if self.config.Q is None:
            if self.config.Q_diag is not None:
                Q = self.Q_slack * np.diag(self.config.Q_diag)
            else:
                Q = self.Q_slack * np.eye(self.config.state_dim)
        else:
            Q = self.config.Q

        # State weights I_N ⊗ Q_w.
        # last diagonal term weighted
        W = np.eye(self.config.HORIZON_LENGTH)
        W[-1, -1] = self.config.TERMINAL_WEIGHT
        QQ = np.kron(W, Q)
        return QQ

    def build_R_trajectory(self, x_bar, u_bar):
        """! Build R along trajectory.
        Notes
        -----
        Can be used to make dynamic range optimization based on rollout (SLAM)
        Kept empty in current iterations.

        @type x_bar: np.ndarray
        @param x_bar: nominal state trajectory
        @type u_bar: np.ndarray
        @param u_bar: Action sequence that generates x_bar

        @rtype: np.ndarray
        @return: Upscaled and weighted action cost matrix (L * action_dim, L*action_dim)
        """
        # Extract single action cost matrix
        if self.config.R is None:
            if self.config.R_diag is not None:
                R = np.diag(self.config.R_diag)
            else:
                R = np.eye(self.config.action_dim)
        else:
            R = self.config.R

        # Action weights: I_N ⊗ R
        RR = np.kron(np.eye(self.config.HORIZON_LENGTH), R)
        return RR

    def continuous_to_binary_mapping(
            self,
            m: int,
            HORIZON_LENGTH: int,
            BIT_MULTIPLIER: int,
            N_BITS: int,
            action_scales: list[float] | None = None,
            scale_multiplier: float|None = None,
    )->tuple[np.ndarray, dict]:
        """! Build binary encoding matrix. Uses first bit as sign bit.

        @type m: int
        @param m: Dimension size for actions (naming is artifact from paper)
        @type HORIZON_LENGTH: int
        @param HORIZON_LENGTH: Length of horizon
        @type BIT_MULTIPLIER: int
        @param BIT_MULTIPLIER: Multiplication factor for whole bit sequence --> K * (2**i)
        @type N_BITS: int
        @param N_BITS: Number of bits  for encoding
        @type action_scales: list[float]
        @param action_scales: Scale for each action
        @type scale_multiplier: float
        @param scale_multiplier: Scale for all actions

        @rtype: np.ndarray
        @return: Binary encoding matrix (m * L * N, m * N) with L the horizon and N the amount of bits
        dict for additional debugging quantities (unused).
        """
        # Setup default scales from config. These are hyperparameters for problem. Defaults to paper values
        if action_scales is None:
            action_scales = [20.0, 2.0] if m == 2 else [1.0] * m
        if scale_multiplier is None:
            scale_multiplier = 1./3.

        # Convert to numpy
        scales = np.array(action_scales) * np.array(scale_multiplier)  # Shape (m,)

        # Bit vector
        bits = np.arange(N_BITS)
        E_base = BIT_MULTIPLIER * (2.0 ** bits)
        E_base = E_base / E_base.max() / 2.0
        # MSB flip for 2's complement style mapping
        E_base[..., -1] = -E_base[..., -1]

        # Scale across dimension
        E_block = np.kron(np.diag(scales), E_base[None, :])

        # Scale across sequence length
        EE = np.kron(np.eye(HORIZON_LENGTH), E_block)
        return EE, {}

    ###################################################################
    # This is unused, but would support variable clipping from config #
    ###################################################################

    # Clipping functions
    def clip_actions(self, actions: np.ndarray) -> np.ndarray:
        return self._clip_bounds(actions,
                           self.config.Q_BOUNDS,
                           self.config.variable_to_action_map)

    def clip_states(self, states: np.ndarray) -> np.ndarray:
        return self._clip_bounds(states,
                           self.config.R_BOUNDS,
                           self.config.variable_to_state_map)

    # General clipping function
    def _clip_bounds(self, arr, bounds, variable_map):
        for variable, bounds in bounds.items():
            # Variable mapping: added complexity for flexibility
            i = variable_map[variable]
            # Clip to bounds
            np.clip(arr[:, i], bounds[0], bounds[1], out=arr[:, i])
        return arr

