...
def plot_results_in_bar_chart(benchmark_dict: dict, output_file: str = "sachi.png", text_type: str = "absolute"):
    """
    ...
    colors = ['#cd87de', '#fff6d5', '#5df3ce', '#2ca02c', '#d62728', '#9467bd', \
    ...
    width = 0.35
    ax[0].bar(x, latency_model, width, label='Model', color=colors[0], edgecolor='black')
    ax[0].bar([i + width for i in x], latency_reported, width, label='Reported', color=colors[1], edgecolor='black')
    ...
    ax[1].bar([i + width for i in x], energy_reported, width, label='Reported', color=colors[1], edgecolor='black')
    ...
    if text_type == "absolute":
        ...
        for i in range(len(benchmark_names)):
            ax[0].text(i+width/2, latency_model[i] + 0.1, f"{int(latency_model[i])}",
                       ha='center', va='bottom', weight='bold')
            ax[1].text(i+width/2, energy_model[i] + 0.1, f"{int(energy_model[i])}",
                       ha='center', va='bottom', weight='bold')
    else:
        # add the text labels to show the modeling mismatch
        for i in range(len(benchmark_names)):
            if energy_reported[i] != 0:
                energy_mismatch = int((energy_model[i] / energy_reported[i] - 1) * 100)  # in percentage
                ax[1].text(i+width/2, energy_model[i] + 0.1, f"{energy_mismatch}%",
                           ha='center', va='bottom', weight='bold')
            latency_mismatch = int((latency_model[i] / latency_reported[i] - 1) * 100)  # in percentage
            ax[0].text(i+width/2, latency_model[i] + 0.1, f"{latency_mismatch}%",
                       ha='center', va='bottom', weight='bold')
