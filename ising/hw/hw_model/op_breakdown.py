...
bars3 = ax.barh(y_pos, spin_updating,
                ...
                label='SU', color='#F7DC6F', edgecolor='black')
bars4 = ax.barh(y_pos, adds,
                left=np.array(off_chip_memory) + np.array(on_chip_memory) + np.array(spin_updating),
                label='ADDs', color='#FFA07A', edgecolor='black')
bars5 = ax.barh(y_pos, comps,
                left=np.array(off_chip_memory) + np.array(on_chip_memory) + np.array(spin_updating) + np.array(adds),
                label='COMPs', color='#98D8C8', edgecolor='black')
bars6 = ax.barh(y_pos, macs,
                left=np.array(off_chip_memory) + np.array(on_chip_memory) + np.array(spin_updating) + np.array(adds) + np.array(comps),
                label='PEs', color='#45B7D1', edgecolor='black')
