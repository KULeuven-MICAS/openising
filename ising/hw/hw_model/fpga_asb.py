...
def fpga_asbv2_hw_model(
    ...
    dram_setting: dict = {"size_gb": 800, "bw": 128, "pj_per_bit": 3.7, "double_buf": True},
    # size_kb: mem size in KB, bw: bit width; area: mm2; @TSMC28nm
    sram_setting: dict = {"size_kb": 160, "bw": 1024, "pj_w_per_bit": 0.04987,
                          "pj_r_per_bit": 0.0587, "area": 0.04975, "double_buf": True},
    # bw: bit width; area: mm2; @TSMC28nm
    local_j_mem_setting: dict = {"size_kb": 80//8, "bw": 32768, "pj_w_per_bit": 0.0157,
                                 "pj_r_per_bit": 0.0089, "area": 0.03636, "double_buf": True},
    num_spins_per_core: int = 1024,  # num of spins each core has, affect the local x/p mem size
    pe_size: dict = {"d1": 32, "d2": 1024, "d3": 2},  # d1: num_pe_cols, d2: num_pe_rows, d3: num_cores
    pj_compute_per_bit: float = 1.25 * ((28/45)**2) * ((0.9)**2),  # pJ/bit@@TSMC28nm, Vdd=0.9V, energy per gate
    mm2_compute_per_bit: float = 0.614*1e-6,  # mm2/bit, also mm2/gate
    tclk: int = 1,  # ns, manually defined
    benchmark_dict: dict = {"name": "Molecular Dynamics", "num_spins": 500,
                            "num_degrees": 8, "num_iterations": 1,
                            "w_pres": 2, "x_pres": 16, "p_pres": 16, "num_trails": 1},
    ...
    local_x_mem_area = pe_size["d3"] * local_x_mem_size_bit * mm2_per_reg  # total area (across cores)
    local_p_mem_area = pe_size["d3"] * local_p_mem_size_bit * mm2_per_reg  # total area (across cores)
    ...
    area_collect = {"sram": sram_setting["area"], "local_j_mem": local_j_mem_setting["area"],
                    "local_x_mem": local_x_mem_area, "local_p_mem": local_p_mem_area, "compute": compute_area}
    ...
    parfor_hw: dict = {"d1": min(workload[mapping["d1"]], pe_size["d1"]),  # num_pe_cols
    ...
    left_workload[mapping["d3"]] = math.ceil(left_workload[mapping["d3"]] / parfor_hw["d3"])
    ...
    temfor_sw: list = [("t", left_workload["t"]), ("i", left_workload["i"]),
                       ("j", left_workload["j"]), ("l", left_workload["l"])]
    ...
    unallocated_loops = copy.deepcopy(temfor_sw[2:])  # neglect the trail and iteration loops
    allocated_loops_total = []
    # derive the allowed loop size on j for local_j_mem
    # num_spins_per_core, parfor_sw[2]: j
    allowed_loop_size_on_j = min(num_spins_per_core / parfor_sw[2][1], unallocated_loops[0][1])
    allocated_loops_on_j: tuple = ("j", allowed_loop_size_on_j)
    unallocated_loops[0] = ("j", math.ceil(unallocated_loops[0][1] / allowed_loop_size_on_j))
    ...
    for mem in mem_list:
        ...
        if mem == "local_j_mem":
            ...
            mem_sizes_bit_min = benchmark_dict["w_pres"] * math.prod(parfor_hw.values()) * \
                math.prod([value for key, value in allocated_loops_total])  # for weight
            allowed_loop_size_on_l = mem_sizes_bit[mem] / allowed_loop_size_on_j / mem_sizes_bit_min
            if allowed_loop_size_on_l > 1:
                unallocated_loop_size_on_l = unallocated_loops[-1][1]  # l
                allocated_loops.append(("l", min(allowed_loop_size_on_l, unallocated_loop_size_on_l)))
                if allowed_loop_size_on_l >= unallocated_loop_size_on_l:
                    unallocated_loops.pop(-1)
                else:
                    unallocated_loops[-1] = ("l", math.ceil(unallocated_loops[-1][1] / allocated_loops[-1][1]))
            logging.info(f"Suitable j mem size (putting entire N*pr J in the mem) is: "
                         f"{unallocated_loop_size_on_l * allowed_loop_size_on_j * mem_sizes_bit_min / 8 / 1024} KB, "
                         f"curr: {local_j_mem_setting["size_kb"]} KB"
                         )
        else:
            if unallocated_loops[0][0] == "j" and unallocated_loops[0][1] > 1:  # x is to be saved in current mem
                mem_sizes_bit_min = benchmark_dict["w_pres"] * math.prod(parfor_hw.values()) * \
                    math.prod([value for key, value in allocated_loops_total]) + \
                    benchmark_dict["x_pres"] * parfor_sw[2][1] * \
                    math.prod([value for key, value in allocated_loops_total if key == "j"])  # for weight and x
                ...
                if allowed_loop_size_on_j > 1:
                    ...
                    if allowed_loop_size_on_j >= unallocated_loop_size_on_j:
                        ...
                        mem_sizes_bit_for_x = unallocated_loop_size_on_j * benchmark_dict["x_pres"] * \
                            parfor_sw[2][1] * \
                            math.prod([value for key, value in allocated_loops_total if key == "j"])
                        mem_sizes_bit_for_l = unallocated_loop_size_on_j * benchmark_dict["w_pres"] \
                            * math.prod(parfor_hw.values()) * \
                            math.prod([value for key, value in allocated_loops_total])  # for weight
                        allowed_loop_size_on_l = (mem_sizes_bit[mem] - mem_sizes_bit_for_x) / mem_sizes_bit_for_l
                        if allowed_loop_size_on_l > 1:
                            ...
                            if allowed_loop_size_on_l >= unallocated_loop_size_on_l:
                                ...
                                unallocated_loops[-1] = ("l",
                                    math.ceil(unallocated_loops[-1][1] / allowed_loop_size_on_l))
                    else:
                        allocated_loops.append(("j", math.ceil(allowed_loop_size_on_j)))
                        unallocated_loops[0] = ("j", math.ceil(unallocated_loops[0][1] / allowed_loop_size_on_j))
            else:  # x is not to be saved in current mem
                mem_sizes_bit_for_l = benchmark_dict["w_pres"] * math.prod(parfor_hw.values()) * \
                    math.prod([value for key, value in allocated_loops_total])  # for weight
                ...
                if allowed_loop_size_on_l > 1:
                    ...
                    if allowed_loop_size_on_l >= unallocated_loop_size_on_l:
                        ...
                        unallocated_loops[0] = ("l", math.ceil(unallocated_loops[0][1] / allowed_loop_size_on_l))
    ...
    for mem_idx in range(len(mem_list)):
        if temfor_hw[mem_list[mem_idx]] == [] or temfor_hw[mem_list[mem_idx]][0][0] != "j":
    ...
    mem_bw_list = [local_j_mem_setting["bw"] * pe_size["d3"], sram_setting["bw"], dram_setting["bw"]]
    mem_dbuf_list = [local_j_mem_setting["double_buf"], sram_setting["double_buf"], dram_setting["double_buf"]]
    ...
    for mem_idx in range(top_mem_idx+1):
        ...
        tmfor_size_x_lower = math.prod(
            [value for mem in mem_list[:mem_idx] for key, value in temfor_hw[mem] if key == "j"])
        tmfor_size_total = math.prod([value for key, value in temfor_sw])  # consider t and i
        tmfor_size_upper = tmfor_size_total / tmfor_size_lower  # tmfor loops below the current mem level
        # calculate the write latency of tmfor_size_lower: wr_from_high
        if mem_idx == top_mem_idx:
            cycles_wr_per_tile = 0
        else:
            if mem_idx != 0 and mem_idx < top_mem_idx_x:  # transmit x. mem_idx != 0: not j mem
                cycles_wr_per_tile = ((tmfor_size_lower * parfor_size * benchmark_dict["w_pres"]) + \
                                      tmfor_size_x_lower * \
                                      parfor_size_x * benchmark_dict["x_pres"]) / (mem_bw_list[mem_idx])
            else:
                cycles_wr_per_tile = (tmfor_size_lower * parfor_size * benchmark_dict["w_pres"]
                                      ) / (mem_bw_list[mem_idx])
        ...
        if mem_idx != 0 and mem_idx < top_mem_idx_x:  # transmit x. mem_idx != 0: not j mem
            cycles_rd_per_tile = ((tmfor_size_lower * parfor_size * benchmark_dict["w_pres"]) + \
                                  tmfor_size_x_lower * parfor_size_x * \
                                  benchmark_dict["x_pres"]) / (mem_bw_list[mem_idx])
        else:
            cycles_rd_per_tile = (
                tmfor_size_lower * parfor_size * benchmark_dict["w_pres"]) / (mem_bw_list[mem_idx])
        ...
        if mem_idx == 0 or mem_idx > top_mem_idx:
            ...
            cycles_wr_per_tile = (tmfor_size_x_lower * parfor_size_x * benchmark_dict["x_pres"]
                                  ) / (mem_bw_list[mem_idx])
            cycles_wr_per_tile = math.ceil(cycles_wr_per_tile)
        ...
        if mem_idx == top_mem_idx:
            ...
            cycles_rd_per_tile = (tmfor_size_x_lower * parfor_size_x * benchmark_dict["x_pres"]
                                  ) / (mem_bw_list[mem_idx])
            cycles_rd_per_tile = math.ceil(cycles_rd_per_tile)
        ...
        if mem_dbuf_list[mem_idx]:
            latency_collect[mem_list[mem_idx]] = max(cycles_wr_from_high, cycles_rd_to_low) + \
                max(cycles_wr_from_low, cycles_rd_to_high)
        else:
            latency_collect[mem_list[mem_idx]] = cycles_wr_from_high + cycles_rd_to_low + \
                cycles_wr_from_low + cycles_rd_to_high
        # calculate the access count
        access_collect[mem_list[mem_idx]] = {"wr": cycles_wr_from_high + cycles_wr_from_low,
                                             "rd": cycles_rd_to_low + cycles_rd_to_high}
        access_collect_detail[mem_list[mem_idx]] = {"wr_from_high": cycles_wr_from_high,
    ...
    if fpga_mode == "v1":
        ...
        separate_x_regfile_size = benchmark_dict["x_pres"] * pe_size["d1"] * pe_size["d3"]
        # core2core_cycle * num_hops per iteration
        cycles_comm = core2core_cycle * num_hops
        # consider t and i
        cycles_comm *= math.prod([value for key, value in temfor_sw if key in ["t", "i"]])
        latency_collect["comm"] = cycles_comm
        area_collect["comm"] = separate_x_regfile_size * mm2_per_reg
        area_total += area_collect["comm"]
    ...
    if fpga_mode == "v1":
        ...
        num_spins_to_update_per_core = parfor_hw["d2"]  # pipelined, see Fig. 3 in the paper
        cycles_per_spin_updating = math.ceil(
            num_spins_to_update_per_core * num_substeps / updating_parallelism_per_core)
        times_of_updating = math.ceil(math.prod([value for key, value in temfor_sw]) / temfor_hw["local_j_mem"][1][1])
        # check if the updating cycle can be hidden by the computation
        if cycles_per_spin_updating <= temfor_hw["local_j_mem"][1][1]:  # hidden by the computation
            cycles_spin_updating = 0
        else:
            cycles_spin_updating = (cycles_per_spin_updating - temfor_hw["local_j_mem"][1][1]) * times_of_updating
    else:
        assert temfor_hw["local_j_mem"][0][0] == "j"
        num_spins_to_update_per_core = parfor_hw["d2"] * temfor_hw["local_j_mem"][0][1]  # num_pe_rows * j
        times_of_updating = math.ceil(math.prod([value for key, value in temfor_sw]
                                                ) / temfor_hw["local_j_mem"][0][1] / temfor_hw["local_j_mem"][1][1])
        cycles_spin_updating = math.ceil(num_spins_to_update_per_core * num_substeps / updating_parallelism_per_core
                                         ) * times_of_updating

    # calculate the energy of step one: computation
    compute_energy = pj_compute_per_bit * parfor_size *  benchmark_dict["w_pres"] * \
        benchmark_dict["x_pres"] * ideal_latency
    energy_collect: dict = {"compute": compute_energy}
    mem_energy_wr_per_bit = {"dram": dram_setting["pj_per_bit"], "sram": sram_setting["pj_w_per_bit"],
                             "local_j_mem": local_j_mem_setting["pj_w_per_bit"]}
    mem_energy_rd_per_bit = {"dram": dram_setting["pj_per_bit"], "sram": sram_setting["pj_r_per_bit"],
    ...
    for mem_idx in range(top_mem_idx+1):
        ...
        for key in ["wr", "rd"]:
            if key == "wr":
                energy_collect[mem][key] = access_collect[mem][key] * mem_bw_list[mem_idx] * mem_energy_wr_per_bit[mem]
            else:
                energy_collect[mem][key] = access_collect[mem][key] * mem_bw_list[mem_idx] * mem_energy_rd_per_bit[mem]
    # calculate the energy of step two: spin updating
    num_cores = pe_size["d3"]
    energy_spin_updating = num_spins_to_update_per_core * num_cores * num_substeps * times_of_updating * \
        pj_compute_per_bit * (benchmark_dict["x_pres"] + benchmark_dict["p_pres"])
    energy_collect["energy_SU"] = energy_spin_updating
    # calculate the overall system latency and energy
    if fpga_mode == "v1":
        latency_system = max(list(latency_collect.values()), cycles_spin_updating)  # spin updating: pipelined
    else:
        latency_system = max(list(latency_collect.values())) + cycles_spin_updating  # spin updating: sequential

    energy_system = sum([sum(energy_collect[mem].values()) for mem in mem_list[:top_mem_idx+1]]
                        ) + compute_energy + energy_spin_updating
    ...
    logging.info(f"Ideal Latency [cycles]: {ideal_latency}, Latency [ns]: {ideal_latency * tclk}, "
                 f"Energy [nJ]: {compute_energy/1000}")
    logging.info(f"System Latency [cycles]: {latency_system}, Latency [ns]: {latency_system * tclk}, "
                 f"Energy [nJ]: {energy_system/1000}")
    logging.info(f"Area [mm2]: {area_collect}, Total Area [mm2]: {area_total}")
    logging.info(f"Top mem: {mem_list[top_mem_idx]}, Top mem for x: {mem_list[top_mem_idx_x]}")
    logging.info(f"System Spec: SRAM size: {sram_setting['size_kb']} KB, "
    ...
    logging.info(f"Local X' reg size: {separate_x_regfile_size/8/1024} KB, PE size: {math.prod(pe_size.values())}, "
    ...
    return (latency_collect, energy_collect, area_collect, latency_system,
            latency_system * tclk, energy_system, area_total)

def plot_results_in_pie_chart(benchmark_name: str, latency_collect: dict, energy_collect: dict, area_collect: dict,
                              fpga_mode: str = "v2", output_file: str = "output/fpga_asb.png"):
    ...
    total_latency = sum(latency_collect.values()) if fpga_mode == "v1" else sum(
        [value for key, value in latency_collect.items() if key != "spin_updating"]) + \
        latency_collect["spin_updating"]
    total_energy = energy_collect["compute"] + \
        sum([sum(energy_collect[mem].values()) for mem in energy_collect.keys() if mem not in [
            "compute", "energy_SU"]]) + energy_collect["energy_SU"]
    ...
    if fpga_mode == "v1":
        ...
        labels = [key for key, value in latency_collect.items() if key != "spin_updating"]
        sizes = [value for key, value in latency_collect.items() if key != "spin_updating"]
        sizes_su = []
        for mem_idx in range(len(labels)):
            if mem_idx == sizes.index(max(sizes)):
                latency_su = [value for key, value in latency_collect.items() if key == "spin_updating"][0]
    ...
    if fpga_mode == "v2":
        ax[0].bar(labels, sizes_su, bottom=sizes, edgecolor='black', label='spin updating (SU)')
    ...
    ax[0].text(max_idx, sizes[max_idx]+latency_su, '\u2605', ha='center', va='center', weight='bold', fontsize=20)
    # add the text labels to show the absolute values
    for i in range(len(labels)):
        if i == max_idx:
            ax[0].text(i, (sizes[i]+latency_su) * 1.01, f"{sizes[i]+latency_su}", \
                       ha='center', va='bottom', weight='bold')
        else:
            ax[0].text(i, sizes[i] * 1.01, f"{sizes[i]}", ha='center', va='bottom', weight='bold')
    ...
    sizes = [energy_collect["compute"]] + [sum(energy_collect[mem].values()) for mem in
             labels[1:-1]] + [energy_collect["energy_SU"]]
    ax[1].pie(sizes, labels=labels, autopct=lambda p: f'{p:.1f}%', startangle=90,
    ...
    ax[2].pie(sizes, labels=labels, autopct=lambda p: f'{p:.1f}%', startangle=90,
              wedgeprops={'edgecolor': 'black'}, textprops={'weight': 'bold'})
    ax[2].axis('equal')
    ax[2].set_title(f'Area [{round(total_area, 2)} mm2]',
                    weight='bold')
    # save the figure
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    """ experiment settings """
    fpga_mode = "v2"
    benchmark_dict = {"name": "Molecular Dynamics", "num_spins": 500, "num_degrees": 8,
                      "num_iterations": 1, "w_pres": 2, "x_pres": 16, "p_pres": 16, "num_trails": 1}
    benchmark_name = benchmark_dict["name"]
    """ experiment settings end """
    latency_collect, energy_collect, area_collect, latency_system, latency_system_in_ns, \
        energy_system, area_total = fpga_asbv2_hw_model(benchmark_dict=benchmark_dict, fpga_mode=fpga_mode)
    plot_results_in_pie_chart(benchmark_name, latency_collect, energy_collect,
                              area_collect, fpga_mode, output_file = "output/fpga_asb.png")
