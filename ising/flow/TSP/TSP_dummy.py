...
def run_TSP_dummy(N_list: list[int], solvers: list[str], args: argparse.Namespace):
    ...
    LOGGER.debug(f"Saving logs in {logpath}")
    ...
    for N in N_list:
        LOGGER.info(f"Running for {N} cities")
        if change_q:
        ...
        for solver in solvers:
            for run in range(nb_runs):
                ...
                if run == nb_runs-1:
                    ...
                    solution_state = return_metadata(fileName=logfile, metadata="solution_state")
                    LOGGER.info(f"Solution state for {solver} is: {solution_state}")
