...
def run_TSP_dummy(N_list: list[int], solvers: list[str], args: argparse.Namespace):
    ...
    problems = {}
    for N in N_list:
        problems[N] = generate_random_TSP(N, seed)
    if use_gurobi:
        logfiles = {}
        for N in N_list:
            logfiles[N] = logpath / f"Gurobi_N{N}.log"
    ...
    for N in N_list:
        ...
        make_solvers_thread(
            solvers, num_iter=num_iter, model=problems[N], nb_runs=nb_runs, logfiles=logfiles, **hyperparameters
        )
